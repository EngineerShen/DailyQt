    ...
    , ui(new Ui::MainWindow)
{
    ...
    _model = new QStandardItemModel(0, 8);
    QStringList list = {"姓名", "性别", "部门", "岗位", "薪资", "婚否", "绩效基数", "合同到期日"};
    ...
    // 代理相关
    _spinboxDelegate = new SpinBoxDelegate();
    ui -> tableView -> setItemDelegateForColumn(4, _spinboxDelegate);

    _ComboBoxDelegate = new ComboBoxDelegate();
    ui -> tableView -> setItemDelegateForColumn(1, _ComboBoxDelegate);
    ui -> tableView -> setItemDelegateForColumn(2, _ComboBoxDelegate);
    ui -> tableView -> setItemDelegateForColumn(3, _ComboBoxDelegate);

    _doubleSpinBoxDelegate = new DoubleSpinBoxDelegate();
    ui -> tableView -> setItemDelegateForColumn(6, _doubleSpinBoxDelegate);

    _calendarDelegate = new CalendarDelegate();
    ui -> tableView -> setItemDelegateForColumn(7, _calendarDelegate);
}
                ...
                if (fields.count() == 8) {
                    ...
                    for (int i = 0; i < 8; i++) {
                        if (i == 5) {
                            ...
                            _model -> setItem(row, i, new QStandardItem(fields[i]));
                        }
                    }
                    row++;
                }
...
void MainWindow::on_actionAdd_triggered()
{
    ...
    _model -> setItem(insert_row, 6, new QStandardItem("1.0"));
    _model -> setItem(insert_row, 7, new QStandardItem("2000-01-01"));
...
void MainWindow::on_actionDel_triggered()
{
    ...
    QList<int> delete_list = rowsToDelete.values();
